...
## Автокомментинг в каналах

Бот умеет автоматически комментировать посты в привязанном к каналу чате (если он добавлен туда как администратор). 

Для управления этой функцией используется переменная окружения `AUTO_COMMENT_ENABLED`.

- **Включение**: `AUTO_COMMENT_ENABLED=true`
- **Особенности**: 
  - Бот реагирует только на автоматические пересылки из канала (системный флаг `is_automatic_forward`).
  - При публикации постов со множеством фотографий (альбомов), Telegram отправляет несколько отдельных сообщений с одинаковым `media_group_id`. Бот кэширует этот ID и комментирует только первую фотографию из альбома, чтобы избежать спама комментариями под каждым фото.
  - Ответ отправляется через реплай (`reply_parameters`) непосредственно на пост в группе.
...
2. **Настройте переменные окружения:**
   ...
   - `AUTO_COMMENT_ENABLED`: Установите `true` для включения автокомментинга постов из привязанного канала.
